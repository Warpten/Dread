...
namespace cppcoro
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            >
            std::suspend_always yield_value(std::remove_reference_t<U>& value) noexcept
            {
    ...
    requires requires (Operation op) {
        { op(std::declval<T>()) } -> std::same_as<void>;
    }
    void forEach(Operation&& operation, generator<T> source) {
        for (auto&& value : source)
            operation(value);
    }

    template <typename T, typename InitialValue, typename Operation>
    requires requires (Operation op, InitialValue i) {
        { op(std::declval<T>(), i) } -> std::same_as<void>;
    }
    auto accumulate(generator<T> source, Operation&& operation, InitialValue&& initialValue) -> std::add_lvalue_reference_t<InitialValue> {
        forEach([operation, &initialValue](auto&& value) mutable { 
            operation(value, initialValue);
        }, std::forward<generator<T>>(source));
        return initialValue;
    }
}

#endif // Generator_hpp__